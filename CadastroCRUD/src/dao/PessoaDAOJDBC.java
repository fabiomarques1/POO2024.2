 ...
 * @author fmacz
 */
public class PessoaDAOJDBC extends DAOGenerico implements PessoaDAO  {
    @Override
    public int inserir(Pessoa pessoa) {
        ...
        int linha = 0;
        try {  
            linha = executarComando(insert, pessoa.getNome(), pessoa.getDataNascimento());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return linha;
    ...
    public int editar(Pessoa pessoa) {
      StringBuilder sqlBuilder = new StringBuilder();
        ...
        int linha = 0;
        try {
            linha = executarComando(update, pessoa.getNome(), pessoa.getDataNascimento(), pessoa.getCodigo());
        } catch (Exception e) {
            e.printStackTrace();
        } 
        return linha; 
